...
const observer = new IntersectionObserver((entries) => {
  entries.forEach((entry) => {
    const element = entry.target.querySelector(".what__title-container");
    const element2 = entry.target.querySelector(".what__img");

    if (entry.isIntersecting) {
      element.classList.add("animation_bt");
      element2.classList.add("animation_bt");
      return; // if we added the class, exit the function
    }

    // We're not intersecting, so remove the class!
    element.classList.remove("animation_bt");
    element2.classList.remove("animation_bt");
  });
});
